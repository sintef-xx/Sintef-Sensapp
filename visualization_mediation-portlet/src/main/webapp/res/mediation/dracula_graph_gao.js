...
Graph.prototype = {
    ...
    addEdge: function(source, target, style) {
        ...
        t.edges.push(edge);
        this.currentEdge=edge ;  //add by gao
    },
...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    this.dragger = function (e) {
        ...
        var pre=select.pop();
        var k=current_key.pop();
        if(pre==null)
        {
        	select.push(this.attrs.text);
        }
        else if(k==17||k=='17')
        {     	
        	var now=this.node.textContent;
        	if(now==null||now==''||now==pre)
        		return;
        	current_graph.addEdge(pre, now, {weight:"1px", directed: true, stroke : "#aaf"});     	
        	//current_layouter.layout();
        	var edge=current_graph.currentEdge;
        	//current_render.draw();
        	edge.connection = current_render.r.connection(edge.source.shape, edge.target.shape, edge.style);
        }
        else
        {
        	...
        	if(n==null||n=='')
        		return;
        	select.push(this.node.textContent);
        }
